...
// ============================================================================
// escapeHtml, formatDateParts, excelToJson, dateToJson, getExcelSerial,
// getDateFromExcel and parseDateString all live in date-utils.js (loaded
// before this file) so they can be unit tested in Node without a DOM.
...
function renderJsonApiResponse(responseObject) {
  ...
  document.documentElement.innerHTML =
    '<head><meta charset="utf-8"><title>SerialDate JSON</title></head>' + '<body style="margin:0">' + '<pre style="margin:0;padding:1rem;font:14px/1.5 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;white-space:pre-wrap;word-break:break-word">' + escapeHtml(json) + "</pre></body>";
}
...
// Utils (getExcelSerial, getDateFromExcel, parseDateString) live in
// date-utils.js, loaded before this file.
...
function updateLanguage() {
  ...
  const flagImg = currentLangSpan.querySelector("#currentFlagImg");
  const textNode = currentLangSpan.childNodes[currentLangSpan.childNodes.length - 1];

  if (flagImg && langMap[currentLang]) {
    flagImg.src = `assets/flags/${langMap[currentLang].flag}.svg`;
...
function getRelativeDate(type) {
  ...
  switch (type) {
    ...
    case "refStartOfNextYear":
      return new Date(today.getFullYear() + 1, 0, 1);
    case "refEndOfNextYear":
...
function renderTables() {
  ...
  const pastRows = ["refToday", "refLastMonday", "refWeekAgo", "refMonthAgo", "refStartOfYear", "refEndOfYear"];

  const futureRows = ["refTomorrow", "refNextMonday", "refInWeek", "refInMonth", "refStartOfNextYear", "refEndOfNextYear"];
